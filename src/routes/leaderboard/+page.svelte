...
<main
	class="fixed h-screen w-screen bg-cover bg-center lg:flex justify-center"
	style="background-image: url('./background/MeleeMenuGreen.png')"
	in:fade={{ delay: 50, duration: 150 }}
	out:fade={{ duration: 300 }}
>
	<div class="place-items-start px-2 md:px-20 w-full min-w-2xl max-w-6xl overflow-scroll">
		...
		<div class="w-full h-12 grid grid-cols-12 col-span-12">
			<div
		...
		<div
			class="w-full place-items-start grid grid-cols-12 overflow-auto bg-black bg-opacity-25"
		>
			<LeaderboardRow
				player={{
					rank: 1,
					nickname: (Math.random() + 1).toString(36).substring(7),
					connectCode: (Math.random() + 1).toString(36).substring(7),
					characters: [Math.floor(Math.random() * 24), Math.floor(Math.random() * 24)],
					rating: 2500,
					winLoss: '1024/32',
				}}
			/>
