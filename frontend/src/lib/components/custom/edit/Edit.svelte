<script lang="ts">
	...
	$: innerWidth = 400;
	$: boardWidth = Math.floor((innerWidth - 160) / 2);
	$: boardHeight = Math.floor((boardWidth / 16) * 9);
		...
		<div class="w-full h-full grid grid-cols-2 px-16 min-w-2xl justify-center">
			...
			<div class={`w-full h-full col-span-1 grid gap-2 justify-center content-center`}>
				...
				<div
					...
					class={`w-[${boardWidth}px] h-[${boardHeight}px] outline outline-4 outline-zinc-700 overflow-hidden shadow-md`}
				>
					<BoardEdit bind:boardHeight bind:layer={selectedLayer} bind:selectedId />
				</div>
				<button
